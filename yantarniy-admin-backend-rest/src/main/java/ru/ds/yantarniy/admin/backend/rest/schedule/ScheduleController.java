...
public class ScheduleController {
    ...
    @GetMapping("/page")
    public ResponseEntity<PageDto<ScheduleDto>> search(@ModelAttribute SearchSchedulesRequestParam requestParam) {
        Page<ScheduleEntity> page = scheduleService.search(mapper.map(requestParam, SearchSchedulesModel.class));
        return ResponseEntity.ok(
                PageDto.<ScheduleDto>builder()
                        .pageNumber(page.getNumber())
                        .pageSize(page.getSize())
                        .totalElements(page.getTotalElements())
                        .first(page.isFirst())
                        .last(page.isLast())
                        .content(mapper.mapAsList(page.getContent(), ScheduleDto.class))
                        .build()
        );
    }
