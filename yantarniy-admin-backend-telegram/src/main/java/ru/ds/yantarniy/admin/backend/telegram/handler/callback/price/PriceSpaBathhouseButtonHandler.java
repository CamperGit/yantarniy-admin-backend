package ru.ds.yantarniy.admin.backend.telegram.handler.callback.price;
...
public class PriceSpaBathhouseButtonHandler implements BotCallbackHandler, ScrollHelperService<PriceEntity> {

    static String PRICE_SPA_LOCATION_EMPTY_MESSAGE_SOURCE = "spa.services.location.empty";
    static String PRICE_SPA_CONTACT_US_MESSAGE_SOURCE = "spa.services.contact-us";

    LocaleMessageSource localeMessageSource;

    ScrollStateHandlerProvider<PriceEntity> provider;
    ...
    @Override
    public InlineKeyboardMarkup getNotEmptyScrollResponseReplyMarkup(ScrollResponse<PriceEntity> scrollResponse) {
        PriceEntity value = scrollResponse.getValue();
        return markupService.getScrollMenuMarkup(
                scrollResponse.getNumberOfItems(),
                value.getId(),
                scrollResponse.getCurrentPosition(),
                CallbackValue.OPEN_SPA_PRICE_BATHHOUSE.getValue(),
                CallbackValue.PRICE_SPA.getValue(),
                localeMessageSource.getMessage(PRICE_SPA_CONTACT_US_MESSAGE_SOURCE),
                CallbackValue.OPEN_PRICE_SPA_CONTACT_US.getValue()
        );
    }

    @Override
    public String getNotEmptyScrollResponseDescription(ScrollResponse<PriceEntity> scrollResponse) {
        return null;
    }

    @Override
    public InputStream getNotEmptyScrollResponseFile(ScrollResponse<PriceEntity> scrollResponse) {
        PriceEntity value = scrollResponse.getValue();
        return Optional.ofNullable(value.getFile()).map(FileEntity::getId).map(fileService::getFileInputStreamById).orElse(null);
    }

    @Override
    public String getNotEmptyScrollResponseFilename(ScrollResponse<PriceEntity> scrollResponse) {
        PriceEntity value = scrollResponse.getValue();
        return Optional.ofNullable(value.getFile()).map(FileEntity::getName).orElse(null);
    }

    @Override
    public ScrollStateHandler<PriceEntity> getScrollStateHandler(ScrollState scrollState) {
        return provider.getScrollHandler(scrollState);
    }

    @Override
    public SpecificationsSearchService<PriceEntity> getSpecificationSearchService() {
        return priceService;
    }

    @Override
    public List<Specification<PriceEntity>> getAdditionalSpecifications() {
        return Collections.singletonList(
                Specifications.inOrReturnNull("location.type", Collections.singletonList(LocationType.BATHHOUSE.getCode()))
        );
    }

    @Override
    public String getEmptyScrollResponseAnswerMessage() {
        return localeMessageSource.getMessage(PRICE_SPA_LOCATION_EMPTY_MESSAGE_SOURCE);
    }

    @Override
    public InlineKeyboardMarkup getEmptyScrollResponseReplyKeyboard() {
        return markupService.getReturnMarkup(CallbackValue.PRICE_SPA.getValue(), true);
    }

    @Override
    public boolean isApplicable(String callback) {
        return callback.contains(CallbackValue.OPEN_SPA_PRICE_BATHHOUSE.getValue());
    }
}
