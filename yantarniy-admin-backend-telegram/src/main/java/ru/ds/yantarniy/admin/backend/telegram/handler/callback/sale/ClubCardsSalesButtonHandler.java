...
public class ClubCardsSalesButtonHandler implements BotCallbackHandler, ScrollHelperService<SaleEntity> {

    static String CLUB_CARDS_SPA_LOCATION_EMPTY_MESSAGE_SOURCE = "sales.club-cards.empty";
    static String CLUB_CARDS_CONTACT_US_MESSAGE_SOURCE = "sales.club-cards.contact-us";
    ...
    public InlineKeyboardMarkup getNotEmptyScrollResponseReplyMarkup(ScrollResponse<SaleEntity> scrollResponse) {
        ...
        return markupService.getScrollMenuMarkup(
                ...
                CallbackValue.OPEN_CLUB_CARDS_SALES.getValue(),
                CallbackValue.OPEN_SALES.getValue(),
                localeMessageSource.getMessage(CLUB_CARDS_CONTACT_US_MESSAGE_SOURCE),
                CallbackValue.OPEN_CLUB_CARDS_CONTACT_US.getValue()
        );
    }
    ...
    public List<Specification<SaleEntity>> getAdditionalSpecifications() {
        return Collections.singletonList(
                Specifications.inOrReturnNull("location.type", Collections.singletonList(LocationType.CLUB_CARDS.getCode()))
        );
    }

    @Override
    public String getEmptyScrollResponseAnswerMessage() {
        return localeMessageSource.getMessage(CLUB_CARDS_SPA_LOCATION_EMPTY_MESSAGE_SOURCE);
    }
    ...
    public boolean isApplicable(String callback) {
        return callback.contains(CallbackValue.OPEN_CLUB_CARDS_SALES.getValue());
    }
}
